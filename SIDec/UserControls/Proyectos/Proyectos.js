...
function infoFileCarta() {
    var name = $("input[ID*='ContentPlaceHolder1_ucCartas_fuLoadCarta']").val();
    $('#ContentPlaceHolder1_ucCartas_lblPdfCarta').hide();
    name = name.substring(name.lastIndexOf('\\') + 1);
    var ext = name.substring(name.lastIndexOf('.') + 1).toLowerCase();
    if (ext !== 'pdf') {
        $('#ContentPlaceHolder1_ucCartas_lblInfoFileCarta').html('');
        $('#ContentPlaceHolder1_ucCartas_txt_ruta_carta').val('');
        $('#ContentPlaceHolder1_ucCartas_lblErrorFileCarta').html('Extensión inválida, solo se permite «pdf»');
        $('#ContentPlaceHolder1_ucCartas_lblErrorFileCarta').attr('title', 'Extensión inválida, solo se permite «pdf»');
    } else {
        $('#ContentPlaceHolder1_ucCartas_lblErrorFileCarta').html('');
        $('#ContentPlaceHolder1_ucCartas_lblErrorFileCarta').attr('title', '');
        $('#ContentPlaceHolder1_ucCartas_lblInfoFileCarta').html(name);
        $('#ContentPlaceHolder1_ucCartas_txt_ruta_carta').val(name);
        Page_ClientValidate('vgFvgProyectoCartaolio');
    }
}