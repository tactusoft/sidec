...
namespace SIDec
{
  public partial class PlanesPFotos : System.Web.UI.Page
  {
    ...
    private const int _numFotos = 6; // cantidad de fotos

    private const string _SOURCEPAGE = "PlanesPFotos";
    string[] ListaFotos;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      {
        ViewState["Indice"] = "";
        //if (Session["PlanesPVerFotos"] == null || string.IsNullOrEmpty(Session["PlanesPVerFotos"].ToString()) || Session["PlanesPVerFotos"].ToString() != "PF_VF")
        //{
    ...
    private void fLoadFotos()
    {
      ...
      ListaFotos = oFile.fGetArchivoPlanesPFotos(oVar.prPlanesPAu.ToString());
      string nombreFoto = "";
      if (ListaFotos != null && ListaFotos.Length > 0)
      {
        foreach (string Foto in ListaFotos)
        {
          if (Foto.Substring(Foto.LastIndexOf("\\") + 1).Length == 6)
          {
            int indexFoto = 0;
            nombreFoto = Path.GetFileName(Foto).Replace(Path.GetExtension(Foto), "");
            ViewState["TipoFoto"] = nombreFoto.Substring(0, 1);
            ViewState["Indice"] = nombreFoto.Substring(1);
            if (ViewState["TipoFoto"].ToString().ToLower() == "e")
              indexFoto = Convert.ToInt16(ViewState["Indice"].ToString()) + 3;
            else
    ...
    protected void lbSubir_Click(object sender, EventArgs e)
    {
      ...
      if (FileUpload1.Visible)    //Subir foto
      {
        if (FileUpload1.HasFile)
        {
          ...
          pathFileTmp = oVar.prTmpImg.ToString() + "\\" + ViewState["TipoFoto"].ToString() + ViewState["Indice"].ToString() + ".tmp";

          bool Resultado = oFile.fVerificarPath(oVar.prPathPlanesP.ToString() + "\\" + oVar.prPlanesPAu.ToString());
          if (Resultado)
          {
    ...
    {
      if (Convert.ToInt16(sIndex) <= 3)
        ViewState["TipoFoto"] = "u";
      else
      {
        ViewState["TipoFoto"] = "E";
        sIndex = (Convert.ToInt16(sIndex) - 3).ToString();
      }

      string pathFile = oVar.prPathPlanesP.ToString() + "\\" + oVar.prPlanesPAu.ToString() + "\\" + ViewState["TipoFoto"] + sIndex;
    ...
    {
      bool b = oPermisos.TienePermisosSP("sp_u_planp");
      if (!b)
        return;
